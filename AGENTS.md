...
- `HandlerConfig.RejectOnFull` is `bool` (defaults to false, non-blocking only) — when true and `max_concurrency` is reached, the message is rejected immediately without waiting for a slot. Disposition follows top-level `drop_unmatched` (true → ack/drop, false → nack). Validation rejects `reject_on_full: true` combined with `blocking: true`. Per-handler `mqsubscriber.messages.rejected` counter tracks rejections
